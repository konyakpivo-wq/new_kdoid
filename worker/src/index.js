...
function cookie(name, value, maxAge, httpOnly = true) {
  return `${name}=${encodeURIComponent(value)}; Max-Age=${maxAge}; Path=/; Secure; SameSite=Lax${httpOnly ? "; HttpOnly" : ""}`;
}
...
async function decryptSession(env, value) {
  ...
  try {
    ...
    const iv = raw.slice(0, 12), cipher = raw.slice(12);
    const key = await keyFromSecret(env.SESSION_SECRET);
...
async function requireAdmin(request, env) {
  const session = await decryptSession(env, readCookie(request, "nka_session"));
  if (!session || session.login !== env.ADMIN_GITHUB_LOGIN) return null;
  return session;
}

async function getFile(env, token, path) {
  const url = `https://api.github.com/repos/${env.CATALOG_OWNER}/${env.CATALOG_REPO}/contents/${path}`;
...
async function addRepository(request, env) {
  ...
  const repoFile = await getFile(env, session.token, "repo.txt");
  const decaFile = await getFile(env, session.token, "deca.txt");
  let repoText = decodeBase64(repoFile.content).replace(/\r/g, "").trim();
  let decaText = decodeBase64(decaFile.content).replace(/\r/g, "").trim();
  ...
  const nextDeca = decaText ? `${decaText}\n${name}|${category}|${description}` : `${name}|${category}|${description}`;

  const commit = async (path, content, sha, message) => {
    const url = `https://api.github.com/repos/${env.CATALOG_OWNER}/${env.CATALOG_REPO}/contents/${path}`;
    const r = await githubFetch(url, session.token, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ message, content: encodeBase64(content), sha, branch: env.CATALOG_BRANCH })
    });
  ...
  try {
    await commit("deca.txt", nextDeca + "\n", decaFile.sha, `Add metadata for ${name}`);
  } catch (e) {
    return json({ error: "repo.txt обновлён, но deca.txt не удалось обновить. Проверьте GitHub." }, 502);
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/login") {
      const state = bytesToB64(crypto.getRandomValues(new Uint8Array(24)));
      const redirect = new URL("https://github.com/login/oauth/authorize");
      redirect.searchParams.set("client_id", env.GITHUB_CLIENT_ID);
      redirect.searchParams.set("redirect_uri", `${url.origin}/callback`);
      redirect.searchParams.set("scope", "public_repo read:user");
    ...
    if (url.pathname === "/callback") {
      ...
      const savedState = readCookie(request, "nka_state");
      ...
      const userResp = await githubFetch("https://api.github.com/user", tokenData.access_token);
      ...
      return new Response(null, { status: 302, headers: { Location: `${env.SITE_URL}/admin-panel.html?github=success`, "Set-Cookie": cookie("nka_session", session, 8 * 60 * 60) + ", " + cookie("nka_state", "", 0) } });
    }

    if (url.pathname === "/logout") return new Response(null, { status: 302, headers: { Location: `${env.SITE_URL}/admin.html`, "Set-Cookie": cookie("nka_session", "", 0) } });

    if (url.pathname === "/api/me") {
      const session = await requireAdmin(request, env);
      return session ? json({ loggedIn: true, login: session.login }) : json({ loggedIn: false }, 401);
    }
